...
function PaymentDetail() {
    ...
    const DetailCard = ({ icon, iconBg, label, children }) => (
        <View className={`mb-4 rounded-xl border ${themedClasses('bg-white border-gray-100 shadow-sm', 'bg-gray-800 border-gray-700')}`}>
            <View className="p-4">
    ...
    const InfoRow = ({ label, value, isHighlight = false }) => (
        <View className="flex-row justify-between items-center py-1">
            <Text variant="caption" className={themedClasses('text-gray-600', 'text-gray-400')}>
                {label}
            </Text>
            <Text
                variant="body"
            ...
            {isLoading ? (
                ...
                <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                    ...
                    <View className="px-4 pt-6 pb-4">
                        <View className={`rounded-2xl border-2  p-6 ${themedClasses('bg-white border-gray-100 shadow-sm', 'bg-gray-800 border-gray-700')}`}>
                            <View className="items-center">
                                <Text variant="caption" className={themedClasses('text-gray-600', 'text-gray-400')}>
                                    {t('paymentDetail.status')}
                                </Text>
                                <Text variant="h3" weight="bold" className={`mt-1`}
                                    style={{ color: getStatusTextColor(userPayment.status) }}
                                >
                                    {t(`paymentDetail.statusLabel.${userPayment.status?.toLowerCase()}`) || userPayment.status?.toUpperCase()}
                                </Text>
                                <Text variant="h2" weight="bold" className="text-orange-500 mt-3">
                                    {formatAmount(userPayment?.paymentBillId?.paymentAmount, currentLanguage)}
                                </Text>
                                <Text variant="caption" className={themedClasses('text-gray-600', 'text-gray-400')}>
                    ...
                    <View className="px-4">
                        ...
                        >
                            <InfoRow
                                ...
                                value={userPayment.paymentBillId?.roomId?.boardingHouseId?.name || 'N/A'}
                            />
                            <InfoRow
                            ...
                            <InfoRow
                                ...
                                value={convertMonthYear(userPayment.paymentBillId?.month, userPayment.paymentBillId?.year, currentLanguage) || 'N/A'}
                            />
                        </DetailCard>
                        ...
                        >
                            <InfoRow
                                ...
                                value={`${userPayment.paymentBillId?.electricalBill?.oldNumber || 0} kWh`}
                            />
                            <InfoRow
                                label={t('paymentDetail.newReading')}
                                value={`${userPayment.paymentBillId?.electricalBill?.newNumber || 0} kWh`}
                            />
                            <InfoRow
                                label={t('paymentDetail.consumption')}
                                value={`${userPayment.paymentBillId?.electricalBill?.quantityConsumed || 0} kWh`}
                            />
                            <View className="border-t border-gray-200 dark:border-gray-600 mt-2 pt-2">
                                <InfoRow
                                    label={t('paymentDetail.electricityAmount')}
                                    value={formatAmount(userPayment.paymentBillId?.electricalBill?.totalAmount, currentLanguage)}
                                    isHighlight={true}
                                />
                            </View>
                        </DetailCard>
                        ...
                        >
                            <InfoRow
                                label={t('paymentDetail.oldReading')}
                                value={`${userPayment.paymentBillId?.waterBill?.oldNumber || 0} m³`}
                            />
                            <InfoRow
                                label={t('paymentDetail.newReading')}
                                value={`${userPayment.paymentBillId?.waterBill?.newNumber || 0} m³`}
                            />
                            <InfoRow
                                label={t('paymentDetail.consumption')}
                                value={`${userPayment.paymentBillId?.waterBill?.quantityConsumed || 0} m³`}
                            />
                            <View className="border-t border-gray-200 dark:border-gray-600 mt-2 pt-2">
                                <InfoRow
                                    label={t('paymentDetail.waterAmount')}
                                    value={formatAmount(userPayment.paymentBillId?.waterBill?.totalAmount, currentLanguage)}
                                    isHighlight={true}
                                />
                            </View>
                        </DetailCard>
                        ...
                        >
                            <InfoRow
                                ...
                                value={formatAmount((userPayment.paymentBillId?.paymentAmount || 0) - (userPayment.paymentBillId?.electricalBill?.totalAmount || 0) - (userPayment.paymentBillId?.waterBill?.totalAmount || 0), currentLanguage)}
                            />
                            <InfoRow
                                label={t('paymentDetail.electricityAmount')}
                                value={formatAmount(userPayment.paymentBillId?.electricalBill?.totalAmount, currentLanguage)}
                            ...
                            <View className="border-t-2 border-orange-200  mt-3 pt-3">
                                <InfoRow
                                    ...
                                    value={formatAmount(userPayment?.paymentAmount, currentLanguage)}
                                    isHighlight={true}
                                />
                            </View>
                        </DetailCard>
                            ...
                            <View className="mt-2 mb-8">
                                ...
                                >
                                    <View className="flex-row items-center">
                                        ...
                                        <Text className="text-white ml-2" variant="body" weight="semibold">
