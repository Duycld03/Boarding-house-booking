...
function Profile() {
  ...
  const [imageUrl, setImageUrl] = useState(DefaultAvatar); // Sử dụng DefaultAvatar làm giá trị khởi tạo
  const [imageError, setImageError] = useState(false); // Thêm state để theo dõi lỗi hình ảnh
  ...
  const handleAvatarChange = (info) => {
    ...
    if (info.file.status === "done") {
      getBase64(info.file.originFileObj, (url) => {
        setLoading(false);
        setImageUrl(url);
        setImageError(false); // Reset lỗi hình ảnh khi có ảnh mới
      });
    }
  };
  ...
  const getUserProfile = async () => {
    try {
      ...
      // Kiểm tra và thiết lập URL ảnh đại diện
                    ...
                    {imageUrl ? (
                      <div className="relative aspect-square w-70 rounded-full overflow-hidden">
                        <img
                          ...
                          onError={handleImageError} // Thêm xử lý lỗi ảnh
                        />
