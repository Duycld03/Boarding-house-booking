...
RoomSchema.statics.updateRoomAvailability = async function (roomId) {
  try {
    ...
    if (boardingHouseType && roomType) {
      ...
      if (newAvailability !== undefined && room.isAvailable !== newAvailability) {
    ...
    for (const room of rooms) {
      ...
      if (boardingHouseType && roomType) {
        ...
        if (newAvailability !== undefined && room.isAvailable !== newAvailability) {
          ...
          updatedCount++;
          console.log(`📝 Queued update for room ${room.roomNumber}: ${room.isAvailable} → ${newAvailability}`);
        }
      } else {
        console.log(`⚠️ Skipping room ${room.roomNumber} - missing required data`);
      }
    }
  ...
  try {
    ...
    if (typeCode === 'nha_tro_truyen_thong' || typeCode === 'mini_house') {
      return {
        ...
        reason: currentRenters > 0 ? 'Room already occupied (traditional/mini house)' : 'Room available',
        maxCapacity: 1,
        currentOccupancy: currentRenters
      };
    }
      ...
      return {
        ...
        reason: currentRenters >= maxPeople ? 'Room at full capacity' : `Room available (${currentRenters}/${maxPeople})`,
...
cron.schedule('0 2 * * *', async () => {
  console.log(`Starting daily availability sync at ${new Date().toISOString()}`);
  try {
    const result = await Room.updateAllRoomsAvailability();
    console.log(`✅ Daily availability sync completed: ${result.updatedCount}/${result.totalProcessed} rooms updated`);
  } catch (error) {
