...
function BoardingHouseTypeManagement() {
  ...
  useEffect(() => {
    if (
      filterValue &&
      (filterValue.name || filterValue.startDate || filterValue.endDate)
    ) {
  ...
  const handleSelectDelete = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(
          response.message || "Failed to soft delete boarding house type."
        );
      }
    } catch (error) {
      toast.error(
    ...
    <div style={{ padding: "20px" }}>
      ...
      <ConfirmModal
        ...
        content={`Are you sure you want to delete "${
          selectedRequest?.name || "this boarding house type"
        }"?`}
