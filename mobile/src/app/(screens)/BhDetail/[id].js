...
import { Text, BoardingHouseGallery, Loader, Line, Button } from '@/components/ui';
import { BackHeader } from '@/components/navigation/CustomHeader';
import { OwnerInfo, Description, RoomTypeCard, ReviewList } from '@/components/screen/bhDetail';
...
export default function BhDetailScreen() {
    ...
    // Memoized values
    const boardingHouseId = useMemo(() =>
        id || DEFAULT_BOARDING_HOUSE_ID, [id]
    );
    ...
    const formattedAddress = useMemo(() => {
        ...
        const { detail, ward, district, province } = data.boardingHouseDetail.address;
        return `${detail}, ${ward}, ${district}, ${province}`;
    }, [data.boardingHouseDetail?.address, t]);

    const priceRangeText = useMemo(() =>
        `${formatAmount(data.boardingHouseDetail?.priceRange)}/${t("roomTypeCard.month")}`,
    ...
    const fetchAllData = useCallback(async () => {
        if (!boardingHouseId) {
            setUi(prev => ({ ...prev, error: t("boardingHouseIdRequired"), loading: false }));
            return;
        }
    ...
    const renderBoardingHouseType = () => (
        <View className="mt-4">
            <Text
                ...
                style={{ color: '#fff' }}
            >
                {data.boardingHouseDetail?.boardingHouseType?.name || t("boardingHouseTypeUnknown")}
            </Text>
        </View>
    );
    ...
    const renderReviewList = () => {
        return (
            <View className="mt-4">
                <Text variant="subtitle" weight="bold">
                    {t('ratingAndReview')} {pagination.totalItems > 0 && `(${pagination.totalItems})`}
                </Text>
                <ReviewList
