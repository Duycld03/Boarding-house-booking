...
function ChangePassword() {
    ...
    const validateForm = () => {
        ...
        if (!formData.oldPassword) {
            newErrors.oldPassword = { message: t("changePassword.oldPassword.required") };
            isValid = false;
        } else if (formData.oldPassword.length < 5) {
            newErrors.oldPassword = { message: t("changePassword.oldPassword.minLength") };
            isValid = false;
        }

        if (!formData.newPassword) {
            newErrors.newPassword = { message: t("changePassword.newPassword.required") };
            isValid = false;
        } else if (formData.newPassword.length < 5) {
            newErrors.newPassword = { message: t("changePassword.newPassword.minLength") };
            isValid = false;
        }

        if (!formData.confirmPassword) {
            newErrors.confirmPassword = { message: t("changePassword.confirmPassword.required") };
            isValid = false;
        } else if (formData.confirmPassword.length < 5) {
            newErrors.confirmPassword = { message: t("changePassword.confirmPassword.minLength") };
            isValid = false;
        } else if (formData.newPassword !== formData.confirmPassword) {
            newErrors.confirmPassword = { message: t("changePassword.confirmPassword.notMatch") };
            isValid = false;
        }
    ...
    return (
        <ScreenContainer
            withPadding={false}
        >
            <BackHeader title={t("changePassword.title")} onBackPress={() => router.back()} />
