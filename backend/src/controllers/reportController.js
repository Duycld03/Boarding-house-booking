  ...
  async filterReviewReports(req, res) {
    try {
      ...
      if (startDate || endDate) {
        const start = startDate ? convertToISODate(startDate) : null;
        const end = endDate ? convertToISODate(endDate) : null;
      ...
      // Bước 4: Lọc theo reason ngay trong query
      if (reason) {
